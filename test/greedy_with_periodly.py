    ...
    def link_augment(self, site, available_link, d, re_links_rate, time_slot):
        ...
        while d > 0:
            ...
            if augment_link is not None:
                ...
                available_augment_cap = link.capacity - link.link_utilization[time_slot] - re_links_rate[augment_link]
                if available_augment_cap >= d:  # 如果最大带宽可满足，则使用最大带宽以下的带宽
                    re_links_rate[augment_link] += d
                    d = 0
                    break
                    ...
                    if d + link.link_utilization[time_slot] + re_links_rate[
                        augment_link] > link.capacity_upperbound:  # 不能以超出物理上限运行
                        available_link_rate = link.capacity_upperbound - link.link_utilization[time_slot] - \
                                                  re_links_rate[augment_link]
                        d -= available_link_rate
                        re_links_rate[augment_link] += available_link_rate
                        ...
                        d = 0
                        break
                else:  # 如果没有免费时间，且最大带宽也满足不了，则使用剩余带宽，并选择下一条链路
